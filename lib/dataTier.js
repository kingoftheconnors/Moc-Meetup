...
async function getClassData(name) {
  ...
  let result = await client.query(
    "SELECT m.days, m.hours, c.title, c.crn FROM class c JOIN meetingTime m ON c.classID = m.classID WHERE LOWER(c.Title) LIKE $1;",
    [name.replace(/\*/g, '%').trim().toLowerCase()]);
  var meetingDicts = {}
  for (let meeting of result.rows) {
    if(!(meeting.title in meetingDicts)) {
      meetingDicts[meeting.title] = []
    }
    meetingDicts[meeting.title].push({"days": meeting['days'], "hours": meeting['hours'], crn: meeting['crn']})
  }
  return meetingDicts;
}

async function getClassesJSON(callback) {
  let result = await client.query("SELECT m.days, m.startTime, m.endTime, m.building, m.room, c.title, c.crn, c.subject, c.level, c.term FROM class c JOIN meetingTime m ON c.classID = m.classID;");
  var meetings = []
  for (let meeting of result.rows) {
...
async function pushClassDataToSheet() {
  getClassesJSON(async function(meetings) {
    // Push data to sheets
    // Identifying which document we'll be accessing/reading from
    var doc = new GoogleSpreadsheet('101ti6D_8qJDQiCQyosh7kSmiGFNQ3XLwdG06zAapPxg');
...
async function deleteOldTerms(maxTerm) {
  ...
  await client.query("DELETE FROM meetingTime WHERE classID IN (SELECT classID FROM class WHERE (term < $1 OR term IS NULL));", [maxTerm], (err, res) => {
    if (err) throw err;
  });
...
function insertClassRow(classTitle) {
  ...
  client.query("INSERT INTO class(title, subject, level, hours, crn, campus, term) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING classID;",
    ...
    for (let meetingTime of classTitle.Meeting) {
      client.query("INSERT INTO meetingTime(days, startTime, endTime, classID, building, room) VALUES ($1, $2, $3, $4, $5, $6);",
        [meetingTime.days, meetingTime.startTime, meetingTime.endTime, res.rows[0]['classid'], meetingTime.building, meetingTime.room], (err2, res2) => {
        if (err2) throw err2;
      });
    }
  });
