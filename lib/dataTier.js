...
async function getRoomsInBuildingFreeDuringTime(building, timeStart, timeEnd, callback) {
  if(client == null) {
    callback(null);
    return;
  }
...
async function getPresets(callback) {
  ...
  let result = await client.query("SELECT p.name, r.classname FROM preset p JOIN presetrow r ON p.presetID = r.presetID;");
  var presetData = {}
...
async function deletePreset(presetName) {
  ...
  await client.query("DELETE FROM meetingTime WHERE presetID IN (SELECT presetID FROM preset WHERE name = $1);", [presetName], (err, res) => {
    if (err) throw err;
  });
  await client.query("DELETE FROM preset WHERE name = $1);", [presetName], (err, res) => {
    if (err) throw err;
  });
}

async function addPreset(presetName, classNames, callback) {
  ...
  for (let row of result.rows) {
    if(row.name.toUpperCase() == presetName.toUpperCase()) {
      callback("Preset with that name already exists! Please try another preset name");
      return;
    }
  }
  client.query("INSERT INTO preset(name) VALUES ($1) RETURNING presetID;",
    [presetName], (err, res) => {
    if (err) {
      throw err;
    }
    ...
    for (let className of classNames) {
      client.query("INSERT INTO presetRow(presetID, classname) VALUES ($1, $2);",
        [res.rows[0]['presetid'], className], (err2, res2) => {
        if (err2) throw err2;
      });
    }
    callback()
