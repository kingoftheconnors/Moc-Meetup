...
async function scrapeUnusedClasses(term) {
    ...
    console.log("Returning ", unusedSubjects.length > 0)
    return unusedSubjects.length > 0
}

async function scrapeData(major, term) {
  console.log("Scraping ", major, " in ", term)
    ...
    while(res) {
      var datas = await page.evaluate((term) => {
          ...
          for (var classNum = 0; classNum < classes.length; classNum++) {
            ...
            var instructor = classes.eq(classNum).children().eq(7).text().match(/(.*?)\s[\(\s].*/)
            var classSize = classes.eq(classNum).children().eq(10).text().match(/.*?\d*? of (\d+).*/)
            ...
            if(meetingTimes.length > 0) {
              titleLinkArray[m] = {
                ...
                Term: term,
                Instructor: (instructor ? instructor[1] : 'Unknown'),
                ClassSize: (classSize ? classSize[1] : '0'),
